...
function updateTable(tableId,productArray){
    ...
    for (i = 0; i < productArray.length; i++) {
        ...
        td4.addEventListener('click',function(){
            processIdSearch(this.parentNode.firstChild.innerHTML);
        });
...
function processTypeSearch(type) {
        return Promise.resolve(api.searchProductsByType(type)).then(function(val) {
...
function processPriceSearch(price) {
        ...
        return Promise.resolve(api.searchProductsByPrice(price, 50)).then(function(val) {
        updateTable('similarTable', val);
    }).catch(function(val) {
        alert(val);
    })
...
document.getElementById("priceSearchButton").addEventListener('click',function(){
